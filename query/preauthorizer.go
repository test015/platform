...
func (a *preAuthorizer) PreAuthorize(ctx context.Context, spec *flux.Spec, auth platform.Authorizer) error {
	...
	for _, readBucketItem := range readBuckets {
		...
		reqPerm, err := platform.NewPermissionAtID(bucket.ID, bucket.Name, platform.ReadAction, platform.BucketsResource)
		if err != nil {
	...
	for _, writeBucketFilter := range writeBuckets {
		...
		reqPerm, err := platform.NewPermissionAtID(bucket.ID, bucket.Name, platform.WriteAction, platform.BucketsResource)
		if err != nil {
